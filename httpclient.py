...
class Request(object):
    ...
    def get_header(self):
        ...
        if self.request.get('host'):
            packet+=self.request['host'] 
        if self.request.get('content_type'):
            packet+=self.request['content_type'] 
        if self.request.get('length'):
            packet+=self.request['length'] 
        return packet
